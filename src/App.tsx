...
function AuroraField({ audioData }: { audioData: React.MutableRefObject<AudioData> }) {
  ...
  useFrame((state, delta) => {
    ...
    const dominant = low > mid && low > high ? "low" : mid > high ? "mid" : "high";
      ...
      for (let i = 0; i < position.count; i += 1) {
        ...
        switch (animStyle) {
          case "Pulse": {
            ...
            const pulse = Math.sin(dist * 0.3 - t * 2) * (0.3 + dominantValue * breathe * 1.5);
          ...
          case "Glitch": {
            ...
            const noise = Math.sin(glitchX * 10 + t * 20) * Math.cos(glitchY * 8 + t * 15);
            const step = Math.floor(Math.sin(t * 10 + glitchX + glitchY) * 3) / 3;
          ...
          case "Chaos": {
            ...
            wave =
              ...
              Math.sin(x * y * 0.05 + t * chaosSpeed * 0.7) * (0.3 + high * 0.3);
            break;
          }
          case "Black Hole": {
          ...
          case "Nebula": {
            ...
            wave =
              ...
              Math.sin(x * 0.7 - drift * 0.5) * Math.sin(y * 0.7 + drift) * 0.4 +
    ...
    barRefs.current.forEach((bar, index) => {
      ...
      const freqValue = data.freqType === "low" ? low : data.freqType === "mid" ? mid : high;
    ...
    coreRefs.current.forEach((core, index) => {
      ...
      const freqValue = data.type === "low" ? low : data.type === "mid" ? mid : high;

      // Pulse and float
      const baseScale = 0.35;
      const targetScale = baseScale + freqValue * 0.6;
      core.scale.setScalar(THREE.MathUtils.lerp(core.scale.x, targetScale, 0.12));
      core.position.y = 2 + Math.sin(t * 1.5 + index * 2) * 0.4 + freqValue * 0.8;
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-slate-950">
      ...
      <main className="contents">
        <audio ref={setAudioElement} src={STREAM_URL} crossOrigin="anonymous" autoPlay={false}>
          <track kind="captions" />
        </audio>
        ...
        <ThemeContext.Provider value={currentTheme}>
          <AnimationContext.Provider value={currentAnimStyle.name}>
            ...
            >
              ...
              <pointLight position={[10, 10, 10]} intensity={1} color={currentTheme.lightPrimary} />
              <pointLight
                position={[-10, 5, -10]}
                intensity={0.5}
